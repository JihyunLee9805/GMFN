...
class dl_func:
    def images_bmp(self, bmp, stride):
        bmp_list = np.array(list(bmp))
        b_list = []
        a = 0
        for i in range(0, len(bmp_list) - 1, 3):
            if i % stride == 0:
                continue
            b_list.append([bmp_list[i + 2], bmp_list[i+1], bmp_list[i]])
        ...
        # lr = np.array(list(bmp))
        # lr_tensor = np2Tensor([lr], opt['rgb_range'])[0]
        # return lr_tensor
        # list = []
        # total_list = []
        # for i in bmp:
    ...
    # def main():
    #     opt = loading_json()
    #     bm_names, test_loaders = loading_image(opt)
    #     solver = loading_solver(opt)
    ...
    #             executing_test(solver, need_HR, batch)

    def loading_image(self, opt):
        bm_names = []
        test_loaders = []
        for _, dataset_opt in sorted(opt['datasets'].items()):
            test_set = dataset(dataset_opt)
            test_loader = dataloader(test_set, dataset_opt)
            test_loaders.append(test_loader)
            print('===> Test Dataset: [%s]   Number of images: [%d]' % (test_set.name(), len(test_set)))
            bm_names.append(test_set.name())

        return bm_names, test_loaders

    def loading_solver(self, opt):
        solver = create_solver(opt)
        return solver

    def loading_images(self, img, opt):
        # img를 가져와야 함
        dataset_opt = opt['datasets']['test_set3']
        test_set = dataset(dataset_opt)
        test_loader = dataloader(test_set, dataset_opt)


    def executing_test(self, solver, need_HR, batch):
