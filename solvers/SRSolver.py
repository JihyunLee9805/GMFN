...
class SRSolver(BaseSolver):
    ...
    def test(self):
        self.model.eval()
        with torch.no_grad(): #기록 추적, 메모리 사용 방지 
            ...
            if self.self_ensemble and not self.is_train:
                SR = self._forward_x8(self.LR, forward_func)#(ensemble true면)solver안 forward_x8사용, (use chop true면) sovler의 over_use_chop 사용
            else:
                print("LR: ",type(self.LR))
                SR = forward_func(self.LR)#netword안 gmfn_archpy의 forward사용 시간 대부분 차지
    ...
    #메모리 소비 줄이기 위해 test 나누기
    def _overlap_crop_forward(self, x, shave=100, min_size=10000000, bic=None):
        print("OVERLAP\n")
        n_GPUs = 8
        scale = self.scale
        b, c, h, w = x.size()
        ...
        if w_size * h_size < min_size:
            ...
            for i in range(0, 8, n_GPUs):
                ...
                sr_batch_temp = self.model(lr_batch)#시간 걸리는 부분-모델에 batch전달
